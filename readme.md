...
- training37(x :1,3 )
  ...
  **区間を分割していくタイプの DP**  
  ...
  前回どこで区切ったのかの場合分け  
  前が同じ色の場合や初めて使う色の場合は注意



- training38(x :1,2,3)
  1. [D - Pair of Balls](https://atcoder.jp/contests/abc216/tasks/abc216_d)  
  シミュレーションで解く問題  
  その色のボールが何個あるかとあるボールの下には何色のボールがあるかを  
  配列にメモしておくことでO(N+M)で実装できる  
  実装にはqueueを使う
  
  2. [A - atcoder < S](https://atcoder.jp/contests/agc048/tasks/agc048_a)  
  場合分けと考察を少し  
  もし最初からatcoder>S ならば 0  
  次にすべての要素がaの場合はどう並び替えても不可能なので-1  
  それ以外の時はS[i]>tならばans = min(ans,i-1)  
  S[i]<=tならばans = min(ans,i)  
  S[i]==aの時は処理を飛ばすことに注意する
  3. [D - Multiple of 2019 ](https://atcoder.jp/contests/abc164/tasks/abc164_d)  
  **「連続する区間」についての問題では、"累積和的なもの" をとるのが定石**   
  今回の問題は後ろからとった  
  ex) 233495の時  
  まず末尾に0を追加する  
  0 -> 5 -> 95 -> 495 -> ... -> 233495  
  7の倍数を探すとき、[4,5]に49がある  
  この時495-5=490も7の倍数である  
  さらに495%7==5%7となる  
  つまり、余りが等しいものの個数を数えその中から  
  2個選ぶ方法の数を足し上げることで求められる
